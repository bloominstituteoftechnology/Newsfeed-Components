...
const articlePanelCreator = (title, date, p1, p2, p3) => {
  ...
  // add class name "article"
  panelDiv.classList.add("article");

  // create panel h2
  const panelTitle = document.createElement("h2");

  // create panel paragraph
  const panelDate = document.createElement("p");
  // add class name "date"
  panelDate.classList.add("date");

  // create three seperate paragraph elements
  ...
  panelDiv.appendChild(panelDate);
  panelDiv.appendChild(paragraphOne);
  panelDiv.appendChild(paragraphTwo);
  panelDiv.appendChild(paragraphThree);
  panelDiv.appendChild(panelExpandBtn);

  // addcontent to the created elements
...
// define parentContainer so you can attach panel Component
const parentComponent = document.querySelector(".articles");

data.forEach(item => {
  const newPanel = articlePanelCreator(
    item.title,
    item.date,
    item.firstParagraph,
...
// // define child you are appending to parentContainer
// const panelComponent = articlePanelCreator();

// // add panel to parentContainer
